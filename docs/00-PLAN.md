...
| 1.7 | Smoke test: `teslafat` binary runs on the dev box, `nbd-client` connects to `/run/teslafat-0.sock`, reads return all-zero from a `NullBackend`, no panics | ~80 (test harness) |
