...
| 6.12 | `setup-lib/12-wifi-stability.sh` — BCM43436 SDIO stability stack. Installs (a) `/etc/NetworkManager/conf.d/10-teslausb-no-powersave.conf` (`wifi.powersave=2` global), (b) `/etc/modprobe.d/brcmfmac.conf` (`options brcmfmac roamoff=1 feature_disable=0x82000`), (c) `/usr/local/sbin/wifi-watchdog.sh` + `wifi-watchdog.{service,timer}` (30 s tick; escalation ladder at fail counts 2/4/6/10 → pause uploads via `/run/teslausb/uploads_paused`, reload brcmfmac, bounce link, reboot). Pure file-install step — 6.10 owns activation. Runtime contract: the cloud-archive uploader honours the pause flag and applies `nice 19` + `IOPRIO_CLASS_IDLE` + `--bwlimit`/`--transfers 1`/`--tpslimit 4`/buffer caps to rclone. See ADR-0022 and `docs/06-OPERATIONS.md` "WiFi stability stack". | ~400 |
| 6.13 | Both scripts pass `shellcheck -S warning` and have a `--help` output that documents every flag | n/a |
