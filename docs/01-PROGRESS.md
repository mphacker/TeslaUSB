...
| 5.8 | `blueprints/lock_chimes.py` + full v1 feature parity (lock chimes + scheduler + groups + template) | ⏳ — **5.8a shipped:** `services/lock_chime_service.py` (836 LOC, full-parity port of v1 with B-1 idioms: WAV validation, multi-pass FFmpeg re-encode, two-pass LUFS loudnorm, atomic publish via `.tmp`+`os.replace`, MD5 helper, path-traversal guards, secure_filename equivalent, frozen+slots dataclass results `WavValidation`/`ReencodeResult`/`ReplaceResult`/`UploadResult`/`DeleteResult`/`ChimeInfo`, typed exceptions `LockChimeAudioError`/`LockChimeFileError`). `ChimesSection` added to TOML config (lock_chime_filename, chimes_folder, size/duration/speed bounds). 30+ new tests (~497 LOC). Gates clean Win + Linux podman: ruff/format/mypy --strict/vulture/bandit; pytest 183p+19s / 89.10% cov / 85% on the new service module. Charter-review: 1 Major (836 LOC > 500-LOC soft ceiling, mitigated by clear sectioning + frozen-dataclass API stability, split deferred to 5.8.x after 5.8b-e). Sub-increments 5.8b-e pending. |
