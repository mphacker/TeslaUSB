...
| 5.9 | `blueprints/light_shows.py` + light_show_service + template (full-parity port; 3 sub-increments 5.9a-c) | ⏳ — **5.9a shipped** ([this commit]): `services/light_show_service.py` (549 LOC) with `LightShowService` class (list/upload_zip/upload_files/delete/bulk_delete/get_active/set_active), frozen-dataclass results (`UploadResult`, `DeleteResult`, `LightShowFile`), typed exceptions (`LightShowError`, `LightShowFileError`), `make_light_show_service` factory. `LightShowsSection` added to config with path-traversal validation. Mode-aware/fsck/partition logic removed (B-1 has no modes). **Net security improvement over v1**: zip handling now streams members with explicit path-traversal rejection instead of v1's `extractall()` (zip-slip vulnerable). 37 service tests + 70 LOC of config tests; 90% cov on new service; 87.01% total. Gates clean. Charter-review APPROVED (1 Major: 549 LOC > 500 soft ceiling — same pattern as 5.8a-d, tracked as 5.9.x cleanup). 5.9b blueprint + 5.9c template pending. |
