...
| 5.4 | Templates skeleton (base.html, partials, theme) | ⏳ in progress — base.html ported with 5 mode-removal edits + media_hub_nav.html partial copied; scaffolding blueprints for `mapping`/`analytics`/`media`/`cloud_archive`/`settings` drafted but not yet wired into `app.py`. |
