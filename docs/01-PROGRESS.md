...
| 5.9 | `blueprints/light_shows.py` + light_show_service + template (full-parity port; 3 sub-increments 5.9a-c) | ⏳ — **5.9a+5.9b shipped** (`f065199` 5.9a service / [this commit] 5.9b blueprint): `services/light_show_service.py` (549 LOC) with `LightShowService` class (list/upload_zip/upload_files/delete/bulk_delete/get_active/set_active), frozen-dataclass results, typed exceptions, `make_light_show_service` factory. `LightShowsSection` added to config. Mode-aware/fsck/partition logic removed. **Net security improvement over v1**: zip-slip-safe streaming extraction. Blueprint `light_shows.py` (585 LOC, 12 routes) wires service via `current_app.extensions["light_show_service"]`; preserves all v1 URL paths verbatim (incl. `<partition>` segments accepted-and-discarded for template compat); cache-invalidator scheduled exactly once per mutation; per-route exception → 400/422/500 mapping. 37 service tests + 52 blueprint tests; 90% / 80% coverage on new files; 86.30% total. Gates clean. Charter-reviews APPROVED (1 Major each: 549/585 LOC > 500 ceiling — tracked as 5.9.x cleanup). 5.9c (template + UI-charter sweep) pending. |
