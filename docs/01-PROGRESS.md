...
| 5.10 | `blueprints/wraps.py` + wrap_service + template (full-parity port; 3 sub-increments 5.10a-c) | ⏳ — **5.10a+5.10b shipped** (`50b8623` 5.10a service / [this commit] 5.10b blueprint): service 470 LOC + blueprint 342 LOC (under 500 ceiling ✅). Tesla constraints enforced verbatim (512–1024 px square PNG only, 1MB, alphanumeric/_/-/space filenames ≤30 chars). `WrapsSection` config added. All v1 URLs preserved (incl. `<partition>` accepted+ignored). B-1 added `/wraps/bulk_delete` for symmetry. Cache-invalidator scheduled exactly once per successful mutation (with clever `total_uploaded > 0` guard on multi-upload). 35 service + 48 blueprint tests; **95% on blueprint** (highest of Phase 5); 87.04% total. Gates clean. Charter-reviews APPROVED. 5.10c (template + UI-charter sweep) pending. |
