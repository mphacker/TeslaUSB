...
| 5.11 | `blueprints/music.py` + music_service + template (3 sub-increments 5.11a-c) | ⏳ — **5.11a shipped** ([this commit]): `services/music_service.py` (645 LOC) with `MusicService` class supporting chunked uploads (resolve_file_path/list_files/upload_files/save_file/handle_chunk/delete_file/bulk_delete/create_directory/delete_directory/move_file/generate_upload_id), 8 frozen-dataclass results, `MusicError`/`MusicFileError` exceptions, `make_music_service` factory. `MusicSection` config (folder/max_file_size/chunk_size/free_space_reserve/stale_chunk_age/allowed_extensions). **Net security improvement over v1**: `.`/`..` path-segment hard-rejection; upload IDs via `secrets.token_hex(16)` (cryptographic). 31 service + 4 config tests; 85% cov on service; 86.72% total. Gates clean (549p+19s). Charter-review APPROVED (1 Major: 645 LOC > 500 soft ceiling; deferred to 5.11.x cleanup). 5.11b blueprint + 5.11c template pending. |
