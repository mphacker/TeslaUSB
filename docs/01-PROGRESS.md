...
| 5.8 | `blueprints/lock_chimes.py` + full v1 feature parity (lock chimes + scheduler + groups + template) | ⏳ — **5.8a-d shipped** (`67414ac`/`9279725`/`84cbeef`/[5.8d]): full service stack (lock_chime_service 836 + chime_group_service 550 + chime_scheduler pkg 1516 across 8 modules) + blueprint with 22 routes (917 LOC) wiring all services through app extensions singletons. Cache-invalidator called exactly once per mutation; service exceptions translated to 400/422; generic `Exception` only at top-level fault isolation. URL signatures preserve v1 verbatim except schedule_id (string UUID, not int — internal-only change). 30+ blueprint tests + 75+ service tests. Gates clean Win+Linux: 322p+20s/325p+17s, 86.52%/86.66% total cov, 80% on new blueprint. Charter-review APPROVED (1 Major: 917 LOC blueprint > 500 ceiling; 22 routes resist clean split). 5.8e (template + security-review skill) pending. |
