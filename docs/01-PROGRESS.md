...
| 5.8 | `blueprints/lock_chimes.py` + full v1 feature parity (lock chimes + scheduler + groups + template) | ⏳ — **5.8a shipped** (`67414ac`): lock_chime_service (836 LOC). **5.8b shipped** (`9279725`): chime_group_service (550 LOC). **5.8c shipped:** `services/chime_scheduler/` package (1516 LOC across 8 modules: types/constants/validation/holidays/formatters/storage/scheduler/__init__) + compat wrapper + 4 schedule types (Weekly/Date/Holiday/Recurring) + priority dispatch (Date>Holiday>Weekly>Recurring) + US fixed+movable holidays + recurring intervals (on_boot/15m/30m/1h/2h/4h/6h/12h) + atomic JSON v1 schema + threading lock + 25+ tests. Bonus: removed only remaining production `# type: ignore` from teslafat_client (5.5 nit follow-up). Gates clean Win+Linux: 273p+19s/276p+16s, 87.72%/87.89% total cov, 75-100% per-file on new code. Charter-review APPROVED (1 Major out-of-scope edit accepted as net-positive; 1 Minor scheduler.py 455>400 sub-target). Sub-increments 5.8d-e pending. |
