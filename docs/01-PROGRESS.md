...
| 5.8 | `blueprints/lock_chimes.py` + full v1 feature parity (lock chimes + scheduler + groups + template) | ⏳ — **5.8a shipped** (`67414ac`): `services/lock_chime_service.py` (836 LOC) + ChimesSection config + 30+ tests, 89.10% cov. **5.8b shipped:** `services/chime_group_service.py` (550 LOC; group CRUD + JSON persistence + atomic write + threading lock + SystemRandom selection + anti-repeat last_selected tracking + path-traversal config validation), PathsSection extended with `state_dir`, factory `make_chime_group_manager(cfg)`. 20+ new tests, gates clean Win + Linux podman (220p/238p / 88.98%/89.25% / 89% on the new service). Charter-review APPROVED (1 Major: 550 LOC > 500 soft ceiling). Sub-increments 5.8c-e pending. |
