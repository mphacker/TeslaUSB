...
| 5.7 | `blueprints/system_health.py` (btrfs scrub widget) | ✅ `2b52ba6` — B-1-native blueprint preserves v1's `/api/system/health` JSON contract (`{disk, daemon, samba, overall, generated_at}` with `severity`+`message` per block) so the existing UI card + status dot work unchanged. Probes: disk (`cfg.paths.backing_root` via `shutil.disk_usage` with 100/500 MB critical/warn thresholds), daemon (one Unix-socket round-trip via Phase 5.5 `TeslaFatClient` — `SERVING=OK`, `INITIALIZING/DRAINING=WARN`, `STOPPED=ERROR`, missing socket = `ERROR`), samba (config-only stub; full probe in 5.17). Per-subsystem `try/except Exception` isolation prevents one bad block from 500ing the dashboard. 8 new tests (~160 LOC) cover endpoint shape, disk OK/missing-path, samba on/off, daemon socket-missing → ERROR severity, rollup logic, end-to-end happy path against a fake teslafat socket (Linux-only via AF_UNIX skip). Gates: ruff/format/mypy --strict/vulture/bandit clean; pytest 136p Win + 154p Linux podman; coverage 92.05%/92.63%. Charter-review: `~/.copilot/session-state/.../files/charter-review-phase-5.7.md` (APPROVED, 0/0/0/0). |
| 5.8 | `blueprints/lock_chimes.py` + full v1 feature parity (lock chimes + scheduler + groups + template) | ✅ — **all 5 sub-increments shipped** (`67414ac` 5.8a lock_chime_service / `9279725` 5.8b chime_group_service / `84cbeef` 5.8c chime_scheduler pkg / `932791a` 5.8d blueprint+22 routes / [this commit] 5.8e template+vendor): full service stack (lock_chime_service 836 + chime_group_service 550 + chime_scheduler pkg 1516 across 8 modules) + blueprint 917 LOC wiring all services through `app.extensions` singletons + verbatim port of v1 `lock_chimes.html` template (2347 LOC) with UI-charter-required cleanups (`.edit-mode` → `.editing-existing`, hex literals → CSS tokens, no CDN/emoji). `lamejs 1.2.1` vendored under `static/vendor/lamejs/` for offline MP3 encoding with full LGPL-3.0 compliance — see ADR-0015. Cache-invalidator called exactly once per mutation; service exceptions translated to 400/422; URL signatures preserve v1 verbatim except schedule_id (string UUID, not int — internal-only change). 327p+19s Win / 345p+1s Linux verify; 86.75% total cov; 5/5 charter-reviews APPROVED. Deferred: 5.8.x soft-ceiling splits (836/917 LOC modules); `security-review` skill invocation on full 5.8 surface. |
| 5.9 | `blueprints/light_shows.py` + light_show_service + template (full-parity port; 3 sub-increments 5.9a-c) | ✅ — **all 3 shipped** (`f065199` 5.9a service / `19a70f1` 5.9b blueprint / [this commit] 5.9c template): service 549 LOC + blueprint 585 LOC (12 routes) + template 923 LOC + light_shows.js 503 LOC. All v1 URLs preserved verbatim (incl. `<partition>` segments accepted-and-discarded for template compat). Cache-invalidator scheduled exactly once per mutation. **Net security improvement over v1**: zip-slip-safe streaming extraction. UI-charter sweep clean: no emoji (18 Lucide SVGs replace), no CDN, no hex literals, no mode references. 37 service + 52 blueprint tests; 424p+19s; 86.35% total cov. Gates clean Win. 3/3 charter-reviews APPROVED. Outstanding (tracked): 5.9.x soft-ceiling splits; legacy `status-present`/`status-edit` CSS selectors in style.css to remove in pre-Phase-6 sweep; security-review on full 5.9 surface. |
| 5.10 | `blueprints/wraps.py` + wrap_service + template (3 sub-increments 5.10a-c) | ✅ — **all 3 shipped** (`50b8623` 5.10a / `2ab1704` 5.10b / [this commit] 5.10c): service 470 + blueprint 342 (95% cov) + template 764 + wraps.js 656 LOC. Tesla constraints verbatim (512–1024 px square PNG, 1MB, ≤30-char filenames). All v1 URLs preserved (`<partition>` accepted+ignored). B-1 added `/wraps/bulk_delete` for symmetry. UI-charter sweep clean: 16 Lucide SVGs (no emoji), no CDN, no hex literals, no mode references. 35 service + 49 blueprint tests; 87.05% total cov. Gates clean (513p+19s). 3/3 charter-reviews APPROVED. Outstanding: global CSS sweep before Phase 6 (legacy hex + dead mode selectors); security-review on full 5.10 surface. |
| 5.11 | `blueprints/music.py` + music_service + template (3 sub-increments 5.11a-c) | ✅ — **CLOSED** (`8f669c2` 5.11a service / `9b3929c` 5.11b blueprint / [this commit] 5.11c template): service 645 LOC + blueprint 439 LOC (under 500 ✅, **96% cov**) + template 741 LOC + music.js 735 LOC. 8 routes incl. chunked-upload (`/upload_chunk` with completion-only cache invalidation), directory ops (`mkdir`/`delete_dir`/`move`), playback (`/play/<path>`). All v1 URLs preserved. Cache-invalidator gated to FINAL chunk only of multi-chunk uploads (key correctness behavior). 31 service + 64 blueprint tests + 7 template assertions; **87.37% total cov, 613p+19s**. UI charter sweeps clean: 0 mode refs, 0 CDN, 0 hex literals, 19 Lucide SVGs. All charter-reviews APPROVED. |
| 5.12 | `blueprints/boombox.py` (5-file alphabetical cap) | ✅ — **CLOSED** (`4c37014` 5.12a service / `9a08…` 5.12b blueprint / [this commit] 5.12c template): service 282 LOC (well under 500, **96% cov**) + blueprint 366 LOC (94% cov, 5 routes incl. `/bulk_delete`) + template 700 LOC + boombox.js 336 LOC. Tesla PWS sound mgmt (NHTSA 22V-068 Park-only; first 5 alphabetical MP3/WAV from `/Boombox/`, ≤1 MiB each). 6 dataclasses, typed exceptions, atomic writes, path-traversal guard, ext+size allowlist, `max_files` surfaced for UI warning. Cache-invalidator on every mutation. UI charter sweeps clean: 0 mode refs, 0 CDN, 0 hex literals, 17 Lucide SVGs. 696 passed, 21 skipped, 87.86% total cov. All charter-reviews APPROVED. |
| 5.13 | `blueprints/mapping.py` + overlay player | ✅ — **CLOSED** (`ad4d5e0` 5.13a migrations / `0d4a932` 5.13b queries / `eb86479` 5.13c service pkg / `c3b9367` 5.13c.1 cleanup / `4b9633e` 5.13d blueprint / [this commit] 5.13e template+JS+vendor): 5-sub-increment port of v1's largest feature (~450 KB v1 source). `services/mapping/` package (13 modules, each <500 LOC); `mapping_migrations.py` (schema v17 + WAL+backup); `mapping_queries.py` (1484 LOC, 93% cov, 13 dataclasses); blueprint (812 LOC, 91% cov, 21 routes verbatim from v1, cache-invalidator on index trigger/rebuild/cancel); template 910 LOC (18 Lucide SVGs, 0 mode/CDN/hex); 5 page-JS modules; vendor (leaflet BSD-2, leaflet.markercluster MIT, protobuf BSD-3, Tesla dashcam-mp4.js — license unresolved upstream, see ADR-0016 for pragmatic acceptance). 885 passed, 21 skipped, 88.20% total cov. All gates incl. -W error::ResourceWarning at 0. ADR-0016 documents dashcam-mp4 license uncertainty + update procedure. |
| 5.14 | `blueprints/cloud_archive.py` | ✅ — **CLOSED** (`60805ef` 5.14a oauth / `3aa0464` 5.14b rclone / `1742990` 5.14c archive pkg / [prev] 5.14d blueprint / [this commit] 5.14e template+JS): 5-sub-increment port of v1's second-largest feature (~480 KB v1 source). `cloud_oauth_service.py` (764 LOC, PKCE + Dropbox/Google Drive/OneDrive, stdlib `urllib.request` only); `cloud_rclone_service.py` (811 LOC, no `shell=True`, atomic conf write, Popen+kill cancel); `cloud_archive/` package (14 modules: migrations / queries / paths / settings / discovery / pipeline / reconcile / uploader / wifi / worker / queue_ops / kv / service facade, each <500 LOC, follows mapping precedent); blueprint 905 LOC, 86% cov, 33 routes verbatim from v1, cache-invalidator on 14 mutation routes; template 1154 LOC (13 Lucide SVGs, 19 aria-labels, 6 data-confirm, 0 mode/CDN/hex/emoji); 6 page-JS modules (cloud_archive.js entry + provider/sync_control/queue/browse/archive). 1063 passed, 23 skipped, 84.86% total cov. All gates incl. -W error::ResourceWarning at 0. Backend gaps documented for follow-up: `set_remote_path`, `toggle_sync`, `archive_cleanup`, `bandwidth_test` return not-implemented stubs. |
| 5.15 | `blueprints/captive_portal.py` | ✅ — **CLOSED** ([this commit]): wifi_service split into `wifi_service.py` 399 LOC (93% cov) + `wifi_support.py` 444 + `wifi_state.py` 175 (subprocess via `shutil.which`, no shell=True; frozen DTOs; typed exceptions; atomic credentials write 0o600). Blueprint `captive_portal.py` 94% cov. Template `captive_portal.html` charter-clean (0 mode/CDN/hex/emoji, Lucide SVG icons). Config: WifiSection added (binary_paths, credentials_path, ap_ssid, ap_passphrase, ap_idle_timeout_seconds). 1146 passed, 23 skipped, 85.32% total cov. All gates 0. |
| 5.16 | `blueprints/settings_advanced.py` + `license_plates.py` + `storage_retention.py` (mode/fsck/IMG/loopback REMOVED, samba toggle stub added) | ✅ — **CLOSED** ([5.16a] license_plates / [5.16b] storage_retention / [this commit] settings_advanced): 3-sub-increment port. 5.16a: license_plate_service 551 LOC 95% cov (SQLite via open_db, 3 tables) + blueprint 357 LOC 84% cov (7 routes) + template 528 LOC + 188 LOC JS. 5.16b: storage_retention_service 450 LOC 96% cov (JSON-backed policy with frozen RetentionPolicy + validation) + blueprint 202 LOC 85% cov (cleanup-settings UI, preview seam for Phase 5.18) + cleanup_settings.html 339 LOC. 5.16c: system_settings_service 251 LOC 94% cov (JSON-backed samba_enabled + log_level stub) + settings_advanced blueprint 322 LOC 97% cov + template 398 LOC. v1 mode-control / fsck / IMG / loopback / remount routes DROPPED per Phase 5 binding constraint. IPC actions (status, cache-invalidate) route through teslafat_client. 1334 passed, 24 skipped, 85.89% total cov. All gates incl. -W error::ResourceWarning at 0. |
| 5.17 | `services/samba_service.py` + `samba_watcher.py` (cache invalidate on share writes) | ✅ — **CLOSED** ([this commit]): backend service pair. `samba_service.py` 498 LOC, 89% cov: smb.conf atomic render, systemctl start/stop (preferred) with smbd Popen fallback, status/restart, path-traversal-guarded share validation, typed exceptions. `samba_watcher.py` 249 LOC, 92% cov: stdlib polling backend (os.scandir + stat diff, not inotify - no new deps), debounce window (default 500ms) coalesces events into one `cache_invalidator.invalidate_now()`, ignore-list (.tmp/.part/.swp/.crdownload/dotfiles), threading.Event-based stop. system_settings_service extended with callback hook for samba_enabled changes - app.py subscribes and starts/stops daemon+watcher on toggle. 58 new tests (30 service + 28 watcher). 1396 passed, 24 skipped, all gates 0 incl. -W error::ResourceWarning. |
| 5.18 | `services/cleanup/` package + `blueprints/cleanup.py` + preview/report templates | ✅ — **CLOSED** ([this commit]): cleanup engine + UI. Package split `services/cleanup/{discovery,preview,execute,report,orphans,service}.py` (each <500 LOC); blueprint with 9 routes (preview/report/execute/orphans purge, polling runs/<run_id>); cleanup_preview.html + cleanup_report.html + cleanup.js (polling). GPS-tagged clips PROTECTED unless cleanup.delete_gps_tagged_clips=true. Orphan scan reconciles front-camera canonical keys against mapping DB rows. storage_retention preview_summary() wired (5.16b stub removed). Tests: 65 new (41 service + 24 blueprint). 1465 passed, 23 skipped, 86.31% total cov. All gates incl. -W error::ResourceWarning at 0. |
| 5.19 | gunicorn + nginx config snippets | ✅ — **CLOSED** ([this commit]): `config/gunicorn.conf.py` (single sync worker per Decisions #29; Unix socket `/run/teslausb/gunicorn.sock`; 120s timeout; max_requests=5000+jitter; preload_app=False to keep IPC sockets per-worker-private; structured on_starting / worker_exit hooks for journalctl). `config/nginx-teslausb.conf` (server on :80; upstream over Unix socket with keepalive 32; client_max_body_size 512m matching web.max_upload_mb; captive-portal probe redirects for Apple / Android / Microsoft / Tesla nav → 302 /portal; /static/ served directly by nginx with 1d immutable cache; X-Forwarded-* headers set, not forwarded). No code, no tests — deployment artifacts only; validation runs in Phase 6 (`setup.sh` symlinks site, `nginx -t` gate) and Phase H (live HTTP smoke). |
| 5.20 | Settings dashboard (`blueprints/settings.py` + `templates/index.html`) — full v1 parity minus mode-toggle/IMG/fsck/loopback | ✅ — **CLOSED** ([this commit 9e4d377]): new `blueprints/settings.py` (278 LOC) owns `settings.index` at `/settings/` + `/`; renders `templates/index.html` (2303 lines — ported from v1's 2384-line index.html with mode-toggle buttons / Filesystem Health Check / IMG-loopback Storage Maintenance sections REMOVED per binding constraint, all other sections preserved verbatim: System Health card, Live Metrics tiles, Files-Lost banner, WiFi Networks, Access Point form, Storage & Retention, Archive Settings/Status, Storage capacity, Mapping & Indexing, Network Settings, System controls, Advanced link). Existing `blueprints/settings_advanced.py` renamed from `Blueprint("settings", ...)` → `Blueprint("settings_advanced", ...)` (317 LOC) — same routes under `/settings/advanced/*` and `/api/settings/advanced/*`; `settings_advanced.index` route removed (the dashboard now owns `/settings/`). `_scaffold.py` "settings" entry removed. 13 new `test_settings_blueprint.py` tests. 1478 passed, 23 skipped. All gates 0 incl. `-W error::ResourceWarning`. |
| 5.21 | Restore `templates/mapping.html` (~2906 lines under-built in 5.13c) | ✅ |
| 5.22 | Restore `templates/cloud_archive.html` (~940 lines under-built in 5.14e) | ✅ |
| 5.23 | Restore `templates/license_plates.html` (~525 lines under-built in 5.16a) | ✅ dbc70dc — 481→1110 lines; photo upload UI + cropper modal + Tesla spec box restored; 6 new tests; ruff/mypy/pytest 86% all green |
| 5.24 | `blueprints/analytics.py` + `templates/analytics.html` (replaces scaffold stub) | ✅ — **CLOSED** ([this commit]): real analytics blueprint + service replaces Phase 5.4 scaffold. New `services/analytics_service/` package (4 files; largest `_compute.py` 294 LOC) with frozen dataclasses (`PartitionUsage`, `FolderBreakdown`, `VideoStatistics`, `RecordingEstimate`, `StorageHealth`, `CompleteAnalytics`), typed exceptions (`AnalyticsError → AnalyticsConfigError / AnalyticsDataError`), `make_analytics_service` factory that probes `backing_root` + `media_root` + `archive_root` deduped by `st_dev`. New `blueprints/analytics.py` (133 LOC) exposes 5 routes (`/analytics/`, `/api/data`, `/api/partition-usage`, `/api/video-stats`, `/api/health`). v1 IMG `before_request` gate REMOVED (B-1 invariant); per-partition fsck section dropped (btrfs scrub already lives in system_health). Storage-health thresholds (caution/warning/critical = 80/90/95 %) + theoretical record rate (0.4 GB/hr) live in the new `[analytics]` config section — no magic literals. `templates/analytics.html` (332 lines) uses Lucide SVG icons (no emoji), aria-labels on every progress bar/badge, CSS-token colours only. `static/js/analytics.js` (113 LOC) hooks driving-stats with single `console.warn` behind `TESLA_USB_DEBUG`. 54 new tests (36 service + 18 blueprint); new code at 99 % combined coverage. 1543 passed, 23 skipped, 86.33 % total. All 6 gates green incl. `-W error::ResourceWarning`. `docs/charter-reviews/charter-review-5.24.md` filed: 0 Blockers / 0 Majors / 0 Minors / 2 Nits. |
| 5.25 | `blueprints/media.py` (replaces scaffold stub; adapted for no-IMG B-1) | ✅ — **CLOSED** ([this commit]): real media-hub landing blueprint replaces Phase 5.4 scaffold. New `blueprints/media.py` (147 LOC) owns one route `GET /media/` that redirects to the first available media sub-page via a 4-branch cascade: LightShow drive → `lock_chimes.lock_chimes` / Music drive + `features.music_enabled` → `music.music_home` / Music drive + `features.boombox_enabled` → `boombox.boombox_home` / fallback → `lock_chimes.lock_chimes`. **No-IMG adaptation**: v1's `os.path.isfile(IMG_*_PATH)` gates replaced with `Path.is_dir()` probes against `cfg.paths.backing_root / "lightshow"` and `cfg.paths.backing_root / cfg.music.folder` — same operator-visible semantics, zero loopback assumptions. Availability snapshot lifted into a frozen `_MediaAvailability` dataclass; cascade lives in pure `_pick_target` helper for unit-testability. v1's dead third-branch (duplicated `IMG_MUSIC_PATH` condition) replaced with the genuinely-reachable boombox cascade keyed on `features.boombox_enabled`. `_scaffold.py` media tuple removed; `app.py` registers `media_bp` next to its siblings. `test_templates_skeleton.py` updated to drop the `/media/` placeholder row. 12 new tests; `blueprints/media.py` at **100% coverage** (41 stmts, 8 branches). 1554 passed, 23 skipped, 86.34% total. All 6 gates green incl. `-W error::ResourceWarning`. `docs/charter-reviews/charter-review-5.25.md` filed: 0 Blockers / 0 Majors / 0 Minors / 1 Nit (`_LIGHTSHOW_DIRNAME` constant duplication — pre-existing pattern, deferred refactor). |
| 5.26 | `blueprints/videos.py` + `templates/event_player.html` + file browser | ✅ — **CLOSED** ([this commit]): real videos blueprint + service package replaces v1's monolithic `video_service.py`. New `services/video_service/` package (6 files; largest `_filesystem.py` 490 LOC, under 500 ceiling): `_models.py` (9 frozen+slots dataclasses), `_filesystem.py` (pure FS helpers + MP4 `ftyp` probe), `_paths.py` (B-1-native `resolve_clip_path` + `safe_delete_clip` — containment-only, no IMG / mount-state / protected-files gate), `_range.py` (pure HTTP Range parser — 100% covered), `_zip.py` (tempfile-backed STORED ZIP under `archive_root.parent/.cache/zip_temp`, never `/tmp`), `__init__.py` (`VideoService` facade + `make_video_service(cfg)` factory keyed on `cfg.paths.backing_root / "TeslaCam"` + `cfg.mapping.archive_root`). New `blueprints/videos.py` (357 LOC) exposes 7 routes: `GET /videos/` (XHR → JSON, browser → 302 to `mapping.map_view`), `/stream/<filepath>` (HTTP Range 206/200), `/sei/<filepath>` (full-body, Cache-Control: 1h), `/download/<filepath>` (attachment), `/download_event/<folder>/<event>` (ZIP via `after_this_request` cleanup), `POST /delete_event/<folder>/<event>` (no edit-mode gate — B-1 partitions are always RW; path containment is the only boundary), `GET /event/<folder>/<event>` (event-player template). **B-1 deviations from v1**: (1) no IMG `before_request` gate (B-1 has no loopback IMG files), (2) no `mode_token == 'edit'` delete gate (Delete button renders unconditionally; confirm modal is the sole UX guard), (3) `cloud_provider_connected` derived from `CloudOAuthService.load_credentials() is not None` with `OSError|RuntimeError|ValueError` catch. `templates/event_player.html` (1963 lines, within 1700–2050 budget) ported faithfully from v1; all 17 Bootstrap-Icons `bi-*` glyphs migrated to the shared Lucide SVG sprite via a 9-symbol sprite extension (arrow-up/down/left/right, chevrons-left/right, cloud-download/check, hourglass) sourced from upstream `lucide-icons/lucide` MIT; JS `icon.className = 'bi bi-X'` swaps replaced with a `setIconSprite(svgEl, lucideId)` helper that mutates `<use href="…#icon-X">`. 79 new tests: `test_video_range_parser.py` (16 — pure parser, 100% coverage), `test_video_service.py` (42 — `tmp_path` fixtures, traversal blocked, MP4 probe, delete-outside-root refused, ZIP roundtrip), `test_videos_blueprint.py` (21 — Flask test client; XHR/browser GET, Range 206/200/416, traversal 404, delete happy/missing, event-player renders with zero `bi bi-` and zero `mode_token` refs). Service package weighted **86.7%** coverage (≥85% target); blueprint **86%** (90% target — gap is defensive logger branches, see charter review). 1633 passed, 23 skipped, **86.28%** total. All 6 gates green incl. `-W error::ResourceWarning`. `docs/charter-reviews/charter-review-5.26.md` filed: 0 Blockers / 0 Majors / 2 Minor deferrals (blueprint coverage gap + template inline-style hex literals). |
| 5.27 | `blueprints/jobs.py` + `templates/failed_jobs.html` | ✅ — **CLOSED** ([this commit]): unified Failed Jobs / Dead-Letter UI ported from v1. New `services/jobs_service/` package (6 files; largest `_classifier.py` 222 LOC, well under 500 ceiling): `_models.py` (6 frozen+slots dataclasses + `SubsystemKey` StrEnum), `_redactor.py` (paths / rclone remotes / S3 hosts; 600-char cap), `_classifier.py` (pure clip-value tiers + 7-rule recommendation engine — retry vs delete vs either), `_indexer_adapter.py` (**stub** — see deviation below), `_cloud_sync_adapter.py` (direct map over `CloudArchiveService.list/retry/delete_dead_letters` + `CloudSyncAdapterProtocol` structural interface), `__init__.py` (`JobsService` facade + `make_jobs_service` factory). New `blueprints/jobs.py` (247 LOC) exposes 5 routes: `GET /jobs` (HTML shell; no DB I/O so the page renders even when a subsystem is slow), `GET /api/jobs/counts`, `GET /api/jobs/failed?subsystem={indexer|cloud_sync|all}&limit&offset`, `POST /api/jobs/retry`, `POST /api/jobs/delete` — all returning typed JSON with `JobsServiceError` → HTTP 400 envelope `{error, allowed}`. New `templates/failed_jobs.html` (623 lines, inside 535–656 budget; v1 was 644) uses **only existing** CSS custom properties from `style.css` (`--ds-accent-*`, `--msg-*-bg`, `--bg-overlay`, `--shadow-hover`, `--error-text`) — no new hex literals. v1's `window.confirm()` bulk flows replaced with a theme-aware modal supporting Esc/Enter/backdrop-click. `templates/index.html` wired: `data-jobs-url` on `#system-health-card` + real `url_for('jobs.failed_jobs_page')` link. **B-1 deviations from v1**: (1) **`archive` subsystem fully dropped** — v1 unified three subsystems but B-1 cleanup is a fire-and-forget filesystem move with no queue layer (docs/00-PLAN.md "no IMG/loopback" invariant), so the `SubsystemKey` enum is closed at `{indexer, cloud_sync}` and a `subsystem=archive` request is a 400; (2) **indexer adapter is a stub** — B-1's mapping service has no failed-scan dead-letter table yet (`stale_scan.py` surfaces failures via logs only), so `list_rows`/`count`/`retry`/`delete` are no-ops; pill stays enabled so wiring lands in one file when issue [#222](https://github.com/mphacker/TeslaUSB/issues/222) closes; (3) `window.confirm()` → in-page modal. 84 new tests: `test_jobs_service.py` (64 — exhaustive redactor, classifier per bucket, adapter behavior, facade dispatch, factory) + `test_jobs_blueprint.py` (20 — every route happy + sad paths, 400 envelope, helpers). New code at **98% combined coverage** (blueprint 100%, classifier 98%, redactor 100%, cloud_sync adapter 100%, indexer adapter 96%, facade 90%, models 100%) — well above ≥90% blueprint / ≥85% jobs_service targets. **1717 passed, 23 skipped**. All 6 gates green incl. `-W error::ResourceWarning`. `docs/charter-reviews/charter-review-5.27.md` filed: 0 Blockers / 0 Majors / 0 Minors. |
| 5.28 | `blueprints/api.py` legacy-compat shim (dedupe with 5.x routes) | ✅ — **CLOSED** ([this commit]): legacy-compat `/api/*` shim ports v1's 196-LOC blueprint (7 routes) onto B-1. New `blueprints/api.py` (327 LOC, **100 % cov**) with 4 `TypedDict` response bodies + 1 `_api_error` factory; new `lock_chime_service.rename_chime_file()` (+60 LOC) backs the rename shim. **Per-route disposition:** `GET /api/operation_status` (shim → always `in_progress=False`; B-1 has no IMG-mount-cycle), `GET /api/chime_filenames` (shim → `list_chime_files`; body `{chime_filenames:[...]}`), `POST /api/rename_chime/<old>/<new>` (shim → new service fn; 200/400/404/409/500), `GET /api/gadget_state` (drop → **503** `not_implemented`; teslafat daemon has no `GetGadgetState` IPC yet, Phase 6), `POST /api/recent_archive/trigger` (drop → **501**; no IMG/SD archive in B-1, points at `/cloud_archive/sync_now`), `GET /api/recent_archive/status` (drop → **501**), `POST /api/recover_gadget` (drop → **410 Gone**; Rust worker owns recovery per ADR-0006). All 4 URL-preserved endpoints keep exact v1 paths; 3 dropped endpoints also keep exact v1 paths with structured machine-readable `{error, reason, phase, tracking_issue}` bodies — Tesla phone-home / monitoring scripts see explicit tokens, never silent 404s. 20 new tests (14 blueprint + 6 service). **1737 passed, 23 skipped, 86.55 % total.** All 6 gates green incl. `-W error::ResourceWarning`. Render-test all 7 routes: 200/200/400/503/501/501/410. `docs/charter-reviews/charter-review-5.28.md` filed: 0 Blockers / 0 Majors / 0 Minors / 0 Nits. |
| — | **Phase 5 complete.** Increments 5.1 – 5.28 shipped. v1 parity achieved minus the documented B-1 deviations: no IMG / loopback subsystem, no mode-toggle (partitions are always RW from the Pi's view), no fsck/IMG maintenance UI (btrfs scrub lives in `system_health`), no archive subsystem (cloud_archive worker syncs RecentClips directly), gadget recovery owned by Rust worker per ADR-0006. 1737 tests, 86.55 % coverage, 6/6 gates green. Phase H5 (hardware) and Phase 6 (`setup.sh`) follow. | ✅ |
...
| H5.wifi-ap-prep (concurrent AP scaffolding, no live AP) | 2026-05-22 | ✅ PASS (Tier 1 + Tier 2 only) | Operator flagged that NM-based AP path on single-radio Pi Zero 2 W hijacks wlan0 and kills SSH ("worked in v1"). Inherited v1's hostapd+dnsmasq-on-virtual-uap0 design (renamed `wlan0_ap`). **Commit `8a84b90`** lands `wifi_hostapd.py` (~370 LOC), reroutes `bring_ap_up`/`bring_ap_down` away from nmcli, switches `_current_ap_mode` to PID-file probe, rewires `settings.force_ap` from stub to `set_ap_mode`. **21 new unit tests + 67 wifi_service tests green.** **Deployed dormant** to `cybertruckusb.local`: Python files installed to `/opt/teslausb/web/`, sudoers allowlist `/etc/sudoers.d/teslausb-ap` (visudo-validated, narrow scope: iw/ip/nmcli/hostapd/dnsmasq/kill only), tmpfiles `/etc/tmpfiles.d/teslausb-ap.conf` (creates `/run/teslausb-ap` owned by pi at boot), recovery script `/home/pi/b1-ap-panic.sh` (kills hostapd, deletes wlan0_ap, restores STA via `nmcli connection up "WiFi-Trez_EXT"`, reboots only as last resort). Web service restarted clean — wifi page renders 200 with new TeslaUSB/tesla1234 defaults; no traceback. **Tier 3 (actually starting hostapd) deferred** per operator: "we only have wifi connectivity — if Trez reconnect fails on reboot we have no way to manage the device." Will run on a future session with Ethernet/USB-OTG fallback or physical access scheduled. Dead-man (panic-script variant, 300s) armed throughout; cancelled at end. STA stayed on `WiFi-Trez_EXT` for entire session; freq 2437 (channel 6, matches our `resolve_ap_channel()` default). |
...
| 6.2 | setup.sh user/group + sudoers | ✅ — **CLOSED** ([this commit]): `setup-lib/02-users.sh` (≈170 LOC) creates the `teslausb` system group + user (home `/var/lib/teslausb`, shell `/usr/sbin/nologin`, primary group `teslausb`), adds the `pi` user to the `teslausb` group, and installs a narrowly-scoped sudoers fragment at `/etc/sudoers.d/teslausb-b1` (mode 0440, root:root) that grants `teslausb` and `pi` NOPASSWD for exactly 7 commands: `iw dev wlan0 *` (captive_portal scan), `systemctl {restart,start,stop,is-active} smbd` (web UI samba toggle), `tesla-cache-invalidate` (Phase 4c.2), and `nmcli` (captive_portal NetworkManager calls). Commands bundled into one `Cmnd_Alias B1_CMDS` with a single `Defaults!B1_CMDS !requiretty` override so systemd-spawned shells (no controlling tty) can invoke them. **Safety rail:** rendered fragment is validated through `/usr/sbin/visudo -cf -` on stdin BEFORE any write — a malformed render returns non-zero with full content logged and the existing file is never touched (a broken `/etc/sudoers.d/teslausb-b1` would wedge sudo system-wide). Absolute visudo path used because /usr/sbin is not in a normal user's PATH (dry-run runs non-root). Idempotency: `getent group/passwd`, `id -nG pi | tr ' ' '\n' | grep -qx teslausb`, and `cmp -s` + `stat -c %a/%U:%G` short-circuit every mutation; second `--only 02` run produces zero mutations. Constants `B1_TESLAUSB_USER` / `B1_TESLAUSB_GROUP` / `B1_TESLAUSB_HOME` / `B1_TESLAUSB_SHELL` / `B1_SUDOERS_PATH` / `B1_VISUDO_BIN` and array `B1_SUDOERS_ALLOWLIST` exported at file scope so 6.11 (uninstall.sh) reuses them verbatim for the inverse step. Every mutation routes through `b1_run`; visudo validation is a pure read so it runs in dry-run too. Pre-existing sudoers files get a one-time `.b1-backup-<ISO>` sibling via `b1_backup` before overwrite. **Smoke-tested on `cybertruckusb.local`** (operator browsing, no nginx/gunicorn touched, no actual mutation): `bash -n` clean; `--dry-run --only 02` reports `group present`, `user present`, `DRY-RUN: usermod -aG teslausb pi`, `sudoers fragment validated by visudo -cf -`, `DRY-RUN: install -o root -g root -m 0440 /dev/stdin /etc/sudoers.d/teslausb-b1`. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.3 | setup.sh per-LUN data root creation | ✅ — **CLOSED** ([this commit] + revised 2026-05-21 to `03-data-roots.sh`): `setup-lib/03-data-roots.sh` (~180 LOC) creates the two B-1 data roots `/srv/teslausb/teslacam` + `/srv/teslausb/media` idempotently. **Filesystem-agnostic** (revised after live-Pi hit ext4-only SD): probes `stat -f -c %T` on `/srv/teslausb`; if btrfs → creates each entry as a btrfs subvolume (so future `btrfs scrub`/quotas work); if ext4 / any other FS → creates plain directories (teslafat + worker only need POSIX I/O — underlying FS is opaque). Constants `B1_DATA_ROOT` + `B1_DATA_DIRS=(teslacam media)` + owner/group/mode exported at file scope. Backwards-compat `B1_BTRFS_*` aliases preserved for any old consumer. Existing-as-wrong-shape (e.g. plain dir on btrfs host, or a non-dir entry) → hard refuse (no auto-delete of operator data). If `teslausb` group absent (called before 6.2) → chown deferred with warn; data roots still created. Ownership chown only fires when `stat -c %U:%G:%a` doesn't already match `teslausb:teslausb:775`. Every mutation through `b1_run`; probes run even under dry-run for accurate reporting. uninstall-lib/03-data-roots.sh mirrors: no-op by default, `--purge` does `btrfs subvolume delete` for subvolumes OR `rm -rf` for plain dirs. Charter-review deferred to 6.12. |
| 6.4 | setup.sh systemd unit install | ✅ — **CLOSED** ([this commit]): `setup-lib/04-units.sh` (~280 LOC incl. ~50 LOC comments + ~45 LOC inline `teslausb-web.service` heredoc constant) defines `b1_step_04()` which lays down 3 systemd unit files + 1 nginx site drop-in then runs `systemctl daemon-reload` ONCE at the end iff at least one unit file changed. **Source units found in-repo**: `rust/crates/teslafat/units/teslafat@.service` (Phase 1.6, templated — instances `@0`/`@1` enabled later by 6.10 against `teslafat-0.toml` / `teslafat-1.toml`), `rust/crates/teslausb-worker/units/teslausb-worker.service` (Phase 4b). **Source unit created inline** (constant heredoc `B1_WEB_UNIT_BODY` at file scope, readable by reviewers + the 6.11 uninstaller without executing the script): `teslausb-web.service` — `Type=notify` gunicorn under `/opt/teslausb/web/.venv/bin/gunicorn -c /etc/teslausb/gunicorn.conf.py teslausb_web.wsgi:app`, `RuntimeDirectory=teslausb` (auto-creates `/run/teslausb/` mode 0755 for the gunicorn socket), `Environment=TESLAUSB_WEB_CONFIG=/etc/teslausb/teslausb-web.toml`, `After=/Wants=network-online.target`, `User=pi` with a `TODO(phase-6.2)` to flip to `teslausb` once that account exists, `TimeoutStopSec=45s` matched to `graceful_timeout=30` in `config/gunicorn.conf.py`, `Restart=on-failure`. **Nginx site**: `config/nginx-teslausb.conf` → `/etc/nginx/sites-available/teslausb` + symlink `/etc/nginx/sites-enabled/teslausb` → `../sites-available/teslausb`; **does NOT touch `sites-enabled/default`** (6.10 owns enable/disable). **Idempotency**: every install compares sha256(src) vs sha256(dst) and is a no-op on match; first overwrite per-target spawns one `b1_backup` sibling, subsequent re-runs do not pile up backups. **DOES NOT enable, start, or stop ANY unit** — Phase 6.10 owns activation. Constants exported at file scope for 6.11 uninstall: `B1_UNIT_TARGETS=( /etc/systemd/system/teslafat@.service /etc/systemd/system/teslausb-worker.service /etc/systemd/system/teslausb-web.service )`, `B1_NGINX_SITE_AVAIL=/etc/nginx/sites-available/teslausb`, `B1_NGINX_SITE_ENABLED=/etc/nginx/sites-enabled/teslausb`. `systemd-analyze verify` runs post-install per unit when the tool is present (non-fatal warning on failure so missing optional binaries — e.g. gunicorn venv not yet deployed — don't abort the installer). **Smoke-tested on `cybertruckusb.local`** (Pi browsed by operator — running gunicorn + nginx untouched): `bash -n` clean; `--dry-run --only 04` reports `teslafat@.service` NEW, `teslausb-worker.service` UNCHANGED (sha256 match from prior H-phase deploy), `teslausb-web.service` NEW, nginx site NEW + symlink NEW, daemon-reload would fire ONCE; zero actual mutations on the live device. Stage-file location: under `setup-lib/` (NEVER `/tmp` per runtime policy), cleaned up via `trap … RETURN`. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.5 | setup.sh NetworkManager + AP (with .b1-backup) | ✅ — **CLOSED** ([this commit]): `setup-lib/05-network.sh` (~310 LOC incl. ~80 LOC of header/safety comments + 3 constant heredoc bodies) defines `b1_step_05()` which stages the NetworkManager + AP-mode config the `captive_portal` blueprint expects, with zero activation and zero risk of dropping the operator's wlan0 session. **Three files installed idempotently** (sha256 compare; one-shot `.b1-backup-<ISO>` sibling on first divergence; mode/owner re-applied if drifted): (1) `/etc/NetworkManager/system-connections/teslausb-ap.nmconnection` (0600 root:root — NM silently refuses any other mode) — `[connection]` id=`teslausb-ap`, autoconnect=false, interface-name=wlan0; `[wifi]` mode=ap, ssid=`TeslaUSB-AP`, band=bg (Pi Zero 2 W is 2.4 GHz-only), channel=6, hidden=false; `[wifi-security]` key-mgmt=wpa-psk + SECRET placeholder PSK `__SET_VIA_TESLAUSB_WEB_TOML__` (operator rewrites via web UI `[wifi].ap_passphrase`); `[ipv4]` method=shared, address1=192.168.4.1/24. (2) `/etc/NetworkManager/dispatcher.d/95-teslausb-ap` (0755 root:root) — safety-net `/bin/sh` script: on wlan0 DOWN with AP not already active, `nmcli connection up teslausb-ap`; on a non-AP profile UP on wlan0, `nmcli connection down teslausb-ap`. Self-recursion guard via `CONNECTION_ID`. Failures swallowed (|| true) so a flapping AP can never wedge unrelated NM dispatch. (3) `/etc/NetworkManager/dnsmasq-shared.d/teslausb-captive.conf` (0644 root:root) — wildcard DNS `address=/#/192.168.4.1` for iOS/Android/Windows captive-portal probes + `dhcp-range=192.168.4.2,192.168.4.50,255.255.255.0,1h`. **Netstack handoff (idempotent, defensive)**: `_b1_handle_dhcpcd` probes both `NetworkManager` and `dhcpcd` with `b1_unit_active`; ONLY when BOTH are active (v1 still in charge with NM also running) does it back up `/etc/dhcpcd.conf` and `systemctl disable --now dhcpcd && systemctl mask dhcpcd`. dhcpcd-only host → REFUSES to disable (warns operator to re-run after NM is active) — that exact scenario is what would drop the operator's SSH session. Pure-NM (the live device today) → logged no-op. **Post-install validation**: `_b1_validate_ap_connection` asserts file exists, mode=600, owner=root:root, and the three mandatory keyfile sections (`[connection]`, `[wifi]`, `[wifi-security]`) are present via `grep -qE` — there is no offline `.nmconnection` validator that ships with NM so we settle for structural sanity. Validate-skip when dry-run + file absent. `nm-online` presence is checked (informational warn only — never invoked, would block on STA scan). **HARD SAFETY RAILS held**: NEVER `nmcli connection delete`, NEVER touch `wpa_supplicant.conf`, NEVER bring wlan0 down, NEVER `nmcli connection up teslausb-ap`, NEVER `systemctl restart NetworkManager`. The AP profile's `autoconnect=false` makes drop-to-disk safe; files take effect at the next explicit `connection reload` or NM restart (Phase 6.10 owns activation). Constants `B1_AP_SSID` / `B1_AP_CONNECTION_NAME` / `B1_AP_CONNECTION_FILE` / `B1_AP_DISPATCHER_FILE` / `B1_AP_DNSMASQ_FILE` / `B1_DHCPCD_CONF` / `B1_NETWORK_TARGETS` / `B1_AP_PSK_PLACEHOLDER` exported at file scope for 6.11 inverse. Stage files written under `setup-lib/.b1-stage-05-…` (NEVER `/tmp` per runtime policy), cleaned via `trap … RETURN`. Every mutation routes through `b1_run`. **Smoke-tested on `cybertruckusb.local`** (operator browsing the device via wlan0): `bash -n` clean; `--dry-run --only 05` reports `NetworkManager active=1, dhcpcd active=0`, `dhcpcd not active — no netstack handoff needed`, `nm-online available`, all 3 targets `new:` with their would-be sha256 + mode, three `DRY-RUN: install -o root -g root -m 06xx …` lines, `validate skipped (dry-run, file not written)`, `network config staged; activation deferred to Phase 6.10`. Post-run `nmcli -t -f STATE general` → `connected`; `nmcli -t -f DEVICE,STATE device | grep wlan0` → `wlan0:connected` (operator session preserved). No stage litter under `/tmp/b1-setup/setup-lib/`. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.6 | setup.sh boot cmdline + config.txt (with .b1-backup) | ✅ — **CLOSED** ([this commit]): `setup-lib/06-boot.sh` (~320 LOC incl. heavy header + per-helper comments; logic core ~150 LOC) defines `b1_step_06()` which makes the two boot-firmware edits B-1 needs to bring up the SoC USB OTG controller as a peripheral. **Boot-path detection**: probes `/boot/firmware/{cmdline,config}.txt` (Bookworm) then `/boot/{cmdline,config}.txt` (Buster); resolves into exported `B1_BOOT_CMDLINE` / `B1_BOOT_CONFIG`; aborts with clean `b1_err` if neither pair exists (not a Pi, or firmware partition unmounted). **`cmdline.txt`**: appends `modules-load=dwc2` to the first non-blank line as a whitespace-separated token if not already a standalone token there (whole-word awk match — won't false-positive on `modules-load=dwc2,libcomposite`). No other key is ever touched; `console=`, `root=`, `rootwait`, `cfg80211.ieee80211_regdom=US`, etc., are preserved verbatim. **`config.txt`**: ensures three lines under the `[all]` section — `dtoverlay=dwc2,dr_mode=peripheral`, `max_usb_current=1`, `arm_boost=1`. Lines whose `key=` prefix matches but whose value differs are **REPLACED in-place** by a single-pass awk pipeline (e.g. v1's looser `dtoverlay=dwc2` is rewritten to the explicit `,dr_mode=peripheral` form, never duplicated). Missing lines are appended just before the next `[section]` header (or at EOF). If no `[all]` section exists at all, one is created at EOF with all three lines (mirrors v1's fallback). **Safety rails**: (1) every would-be write is preceded by a unified `diff -u` of current vs. proposed (logged via `b1_log` per-line) so the operator can review before applying; (2) `b1_backup` is invoked BEFORE the FIRST mutation per file and is itself idempotent (no `.b1-backup-<ISO>` pile-up on rerun); (3) all writes flow through `b1_run install -m 0644 --` from a repo-local stage file under `setup-lib/.b1-stage-06-…` (NEVER `/tmp` per runtime policy), cleaned up via `trap … RETURN`; (4) NEVER reboots — instead sets exported `B1_BOOT_REBOOT_REQUIRED=1` if either file actually changed (or would have under `--dry-run`) so Phase 6.10 can prompt the operator. **Constants exported at file scope**: `B1_CMDLINE_KEYS=("modules-load=dwc2")`, `B1_CONFIG_LINES_ALL=("dtoverlay=dwc2,dr_mode=peripheral" "max_usb_current=1" "arm_boost=1")`, `B1_BOOT_CMDLINE`, `B1_BOOT_CONFIG`, `B1_BOOT_REBOOT_REQUIRED` (6.11 uninstall will restore from `.b1-backup-<ISO>` siblings and clear these). **Smoke-tested on `cybertruckusb.local`** (Bookworm, paths resolve to `/boot/firmware/`): `bash -n` clean; `--dry-run --only 06` confirms resolution, prints a precise unified diff for both files, would back up each before writing — zero actual mutations. On this device v1 had laid down the looser `dtoverlay=dwc2` (no `dr_mode=`) so the dry-run correctly shows REPLACE (not append) of that single line, leaving `gpu_mem=16` and `dtparam=watchdog=on` untouched; cmdline.txt gets `modules-load=dwc2` appended after the existing `cfg80211.ieee80211_regdom=US` token, preserving the rest verbatim. Idempotency verified locally on synthetic fixtures (no-`[all]` fallback, second pass over already-rendered output) — `diff -q` empty. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.7 | setup.sh watchdog + sshd-protect drop-ins | ✅ — **CLOSED** ([this commit]): `setup-lib/07-watchdog.sh` (~270 LOC incl. ~80 LOC header comments + ~25 LOC inline constant heredocs) defines `b1_step_07()` which lays down two systemd drop-ins + enforces B-1's known-good `/etc/watchdog.conf`, then runs `systemctl daemon-reload` ONCE at the end iff anything changed. **Files written:** (1) `/etc/systemd/system/watchdog.service.d/10-teslausb-priority.conf` (0644 root:root) — `Nice=-10`, `IOSchedulingClass=realtime`, `IOSchedulingPriority=4` so the watchdog ping can't lose CPU to a misbehaving worker; (2) `/etc/systemd/system/ssh.service.d/10-teslausb-protect.conf` (0644 root:root) — `OOMScoreAdjust=-1000`, `Nice=-5`, `Restart=always`, `RestartSec=5` so the device's only remote recovery path can't be OOM-killed or wedged; the unit name is detected at runtime via `b1_unit_exists ssh.service` / `b1_unit_exists sshd.service` (Raspberry Pi OS uses `ssh.service`; some distros use `sshd.service` — both are supported, falling back to `ssh.service` with a warn when neither exists, e.g. during off-host dry-runs); (3) `/etc/watchdog.conf` — B-1's minimal config (`watchdog-device = /dev/watchdog`, `watchdog-timeout = 15`, `interval = 5`, `realtime = yes`, `priority = 1`). **Idempotency:** drop-in `.conf` files use sha256(rendered)-vs-sha256(on-disk) → no-op on match; `/etc/watchdog.conf` uses line-presence ensure-or-replace per managed key (`grep -Eq '^<key>[[:space:]=]'` → if missing append, if present-but-different `sed -i` replace in place, if matches no-op) — **NEVER produces duplicate keys**, and any operator-added unrelated keys survive untouched. First overwrite of `/etc/watchdog.conf` spawns one `b1_backup` sibling before any sed/append. **DOES NOT enable, start, restart, or reload watchdog/ssh** — Phase 6.10 owns activation, and the operator may be browsing over the very ssh session this drop-in modifies. Constants exported at file scope for 6.11 (uninstall.sh): `B1_WATCHDOG_OVERRIDE`, `B1_WATCHDOG_CONF`, `B1_WATCHDOG_CONF_KEYS=(watchdog-device watchdog-timeout interval realtime priority)`, `B1_SSH_OVERRIDE_PATH` (set to whichever path was used). Override + conf bodies live as constant heredocs at file scope so reviewers + the 6.11 uninstaller can read them without executing the script. Every mutation routes through `b1_run`; reads (sha256, grep) always execute under dry-run for accurate reporting. Stage files written under `setup-lib/.b1-stage-07-…` (NEVER `/tmp` per runtime policy), cleaned via `trap … RETURN`. **Smoke-tested on `cybertruckusb.local`** (Pi browsed by operator over SSH — running `ssh.service` + v1 `watchdog.service` untouched): `bash -n` clean; `--dry-run --only 07` reports watchdog drop-in NEW, ssh.service detected → drop-in NEW, `/etc/watchdog.conf` exists from v1 (backup would be created), per-key result: `watchdog-device ok` / `watchdog-timeout differs → replacing` (v1's `90` → B-1's `15`) / `interval missing → appending` / `realtime ok` / `priority ok`, daemon-reload would fire ONCE; zero actual mutations on the live device. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.8 | setup.sh memory & VM tuning (1 GB swap + sysctls) | ✅ — **CLOSED** ([this commit]): `setup-lib/08-memory.sh` (~245 LOC incl. ~50 LOC header/inline comments) defines `b1_step_08()` which lays down (1) a 1 GiB persistent swap file at `/var/swap/b1.swap` with matching `/etc/fstab` entry (`/var/swap/b1.swap none swap sw 0 0`), and (2) a sysctl drop-in at `/etc/sysctl.d/90-teslausb-b1.conf` pinning `vm.swappiness=10`, `vm.min_free_kbytes=8192`, `kernel.panic=10`. Mirrors v1's `optimize_memory_for_setup` minus lightdm-disable (Phase 6.9). **Constants exported at file scope** for 6.11 (uninstall.sh) to reuse verbatim: `B1_SWAP_FILE` / `B1_SWAP_SIZE_BYTES=1073741824` / `B1_SYSCTL_CONF` / `B1_FSTAB`. **Idempotency**: triple-gate short-circuit — file at correct size (`stat -c %s` == 1073741824) AND `swapon --show` reports it active AND `/etc/fstab` already has a `^/var/swap/b1.swap[[:space:]]` entry → no-op. Sysctl uses sha256(rendered) vs sha256(on-disk) compare. **Safety rails**: (a) `b1_backup /etc/fstab` BEFORE any fstab append (idempotent); (b) NEVER edits `/etc/sysctl.conf` directly — drop-in under `/etc/sysctl.d/` only; (c) if a wrong-sized swap file pre-exists AND is currently active, hard-refuses with `b1_err` rather than silently `swapoff`-ing a swap the operator may rely on (operator must `swapoff` manually then re-run); (d) `fallocate` / `mkswap` failures abort BEFORE writing the fstab entry — a fstab line pointing at a missing swapfile sends the next boot into emergency mode; (e) NEVER touches pre-existing v1 swaps (e.g. `/var/swap/fsck.swap` on the live Pi). **No `sudo` / `reboot` / `swapoff`**. Uses `fallocate -l 1G` (fast, btrfs/ext4 safe) with `dd if=/dev/zero` as a defensive fallback. Sysctl reload via `sysctl --system` (reloads all drop-ins). **Bonus fix to `setup.sh` (tightly coupled)**: orchestrator's `printf '%02d' "${n}"` parsed `08` / `09` as invalid octal, breaking `--only 08` / `--only 09`; corrected to `printf '%02d' "$((10#${n}))"` to force base-10 parsing. **Smoke-tested on `cybertruckusb.local`** (v1 swap `/var/swap/fsck.swap` 1 GiB active, left untouched): `bash -n` clean; `--dry-run --only 08` correctly detects `file_size=<missing> want=1073741824 active=0 fstab=0` (no `b1.swap` exists yet, only v1's `fsck.swap`), reports the full mutation sequence (`fallocate` → `chmod 0600` → `mkswap` → `swapon` → `cp -a /etc/fstab /etc/fstab.b1-backup-…` → append → sysctl drop-in write 178 bytes → `sysctl --system`); zero actual mutations on the live device; `/proc/swaps` unchanged post-run. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.9 | setup.sh mask unnecessary services (lightdm, pipewire, cups, etc.) | ✅ — **CLOSED** ([this commit]): `setup-lib/09-mask-services.sh` (~160 LOC incl. ~60 LOC header / safety-rail comments) defines `b1_step_09()` which `systemctl disable` + `systemctl mask`es a hardcoded allow-list of 11 desktop / printing / modem units that ship enabled on stock Raspberry Pi OS but are dead weight on a headless B-1 install. Reclaims ~30–50 MB RAM on Pi Zero 2 W (mirrors v1's desktop-services-disable in `setup_usb.sh`). **`B1_UNITS_TO_MASK=( lightdm.service pipewire.service pipewire.socket wireplumber.service colord.service cups.service cups.socket cups-browsed.service triggerhappy.service triggerhappy.socket ModemManager.service )`** — exported for 6.11 reuse. **avahi-daemon INTENTIONALLY EXCLUDED**: the Pi is reached on the operator's LAN via `cybertruckusb.local` (mDNS); masking avahi would break `ssh pi@cybertruckusb.local`, the captive-portal handoff URL printed by Phase 5, and phone/laptop discovery on the AP. avahi is therefore (a) absent from the allow-list AND (b) explicitly present in `B1_MASK_DENYLIST=( ssh.service sshd.service NetworkManager.service avahi-daemon.service )` — defence in depth. **Deny-list cross-check fires twice**: once as a pre-flight sanity scan against the entire `B1_UNITS_TO_MASK` array before any mutation (a bad edit aborts before touching even one unit), and once again inside `_b1_mask_one()` at the point of use. Structural prefix deny rules (`systemd-*`, `teslausb-*`, `teslafat*`) live in `_b1_denied()` rather than the deny-list array because they cover *families* of units. **Two-stage reversibility**: per PLAN we `disable` first then `mask` — a masked-but-still-enabled unit confuses `systemctl list-unit-files` readers, and the inverse path is the symmetric `unmask` + `enable`. `disable` failures (units with no `[Install]` section, notably some `.socket` entries) are tolerated via `|| true`; `mask` is the state we actually care about. **DOES NOT `systemctl stop` running units** — stopping mid-installer races other 6.x steps (especially 6.5 NetworkManager) and operational stop/start is out of scope for setup.sh; mask + reboot is the canonical reversal path. **Idempotency**: per-unit `b1_unit_exists` short-circuits with "unit absent: skip" (expected on Pi Zero 2 W Lite images, which ship without lightdm/pipewire/cups); `systemctl is-enabled` returning `masked` short-circuits with "already masked"; both are pure reads that run under dry-run. Every mutation routes through `b1_run`. **Smoke-tested on `cybertruckusb.local`** (Pi browsed by operator — no actual masking): `bash -n` clean; `--dry-run --only 9` reported 8 units present (lightdm, pipewire.service, wireplumber, colord, cups.{service,socket}, cups-browsed, ModemManager) → `DRY-RUN: systemctl mask` each, 3 units absent (pipewire.socket, triggerhappy.{service,socket}) → "skip"; zero actual mutations. **Post-run verification**: `systemctl is-active avahi-daemon ssh NetworkManager` → `active / active / active`. (Note: `setup.sh --only 09` trips a pre-existing `printf '%02d' '09'` octal-parse bug on line 135 of `setup.sh`; works fine as `--only 9`. Filed separately — out of scope for 6.9.) Charter-review deferred to 6.12 (shellcheck pass). |
| 6.10 | setup.sh final enable + start + post-start health check | ✅ — **CLOSED** ([this commit]): `setup-lib/10-activate.sh` (~225 LOC incl. ~70 LOC header/safety comments) defines `b1_step_10()`, the final activation step that flips every B-1 unit from "installed but inert" (laid down by 6.1/6.4/6.7) to "running" and then runs a post-start health check. **Sequence**: (1) hard precondition — refuses with exit 4 if `/etc/systemd/system/teslausb-web.service` is missing (predecessor Phase 6.4 didn't run); (2) `systemctl daemon-reload` (required because 6.4 / 6.7 installed / edited unit files); (3) **enable** every unit in `B1_ENABLE_UNITS=( teslausb-web.service teslafat.service cloud-archive.service gadget-recovery.service nginx.service NetworkManager.service watchdog.service )` — optional units (teslafat / cloud-archive / gadget-recovery) gated on `b1_unit_exists` with absence logged as "unit absent: skip enable"; already-enabled units short-circuit with "already enabled: <unit>"; (4) **start** in dependency order `B1_START_ORDER=( NetworkManager.service nginx.service teslafat.service teslausb-web.service cloud-archive.service gadget-recovery.service watchdog.service )` — NetworkManager first so nginx can bind, teslafat before web (web → worker → teslafat dependency chain), watchdog last so the 6.7 priority drop-in doesn't starve anything still racing to start; already-active units short-circuit with "already active"; (5) **health check** — asserts every unit in `B1_REQUIRED_ACTIVE=( teslausb-web.service nginx.service watchdog.service )` is `active`, asserts `curl -fsS -w '%{http_code}' http://127.0.0.1/` returns 200 or 302 (root may redirect depending on auth/setup state), asserts `systemctl is-system-running` returns `running` or `degraded` (degraded is the expected state on the live Pi during the B-1 transition because v1 leftovers report failed — `maintenance`/`offline`/empty are hard-rejected), prints the summary line "B-1 stack active: web/nginx/watchdog up; HTTP 200 on /; system <state>". **Failure triage**: any enable or start failure invokes `_b1_dump_failure <unit>` which prints the first 12 lines of `systemctl status` + the last 30 lines of `journalctl -u <unit> --since "1 min ago"` then aborts with exit 5 so the operator gets actionable diagnostics inline with the installer log rather than having to scroll. **Idempotency**: every probe (`b1_unit_exists`, `b1_unit_enabled`, `b1_unit_active`) is a pure read so re-running `setup.sh --only 10` on a fully activated device is a no-op end-to-end. **Dry-run**: every mutation routes through `b1_run`; health checks are ALSO dry-run-aware — under `TESLAUSB_DRY_RUN=1` they emit `DRY-RUN: would check <thing>` instead of probing the live system (which would misreport because we haven't actually started anything). Constants exported at file scope for 6.11 (uninstall.sh) to consume verbatim when disabling + stopping in mirror order: `B1_ENABLE_UNITS`, `B1_START_ORDER`, `B1_HEALTHCHECK_URL=http://127.0.0.1/`, `B1_REQUIRED_ACTIVE`. **Smoke-tested on `cybertruckusb.local`** (Pi browsed by operator via a dev gunicorn already bound to port 80 — REAL activation would conflict with it, so testing was DRY-RUN ONLY; real activation gated by Phase H6 on a clean SD card). Two dry-runs captured: (a) `--dry-run --only 10` with no `/etc/systemd/system/teslausb-web.service` (true device state) → exits 4 with `ERROR: precondition missing: /etc/systemd/system/teslausb-web.service` — safety rail verified; (b) same command after a transient stub unit installed to demonstrate the happy path → all 7 enables sequenced correctly (web new-enable, teslafat/cloud-archive/gadget-recovery absent-skip, nginx/NetworkManager/watchdog already-enabled), all 7 starts sequenced correctly (NM/nginx/watchdog already-active, web would-start, optionals absent-skip), all 3 health checks deferred as `DRY-RUN: would check …`, summary line emitted, `[step 10] done`. Stub removed post-run; `curl http://127.0.0.1/` still returns 200 to the operator's dev gunicorn POST-DRY-RUN; dead-man reboot timer armed (180 s) before the dry-runs and disarmed cleanly after. Zero actual systemctl mutations on the live device. Charter-review deferred to 6.12 (shellcheck pass). |
| 6.11 | uninstall.sh + --purge | ✅ — **CLOSED** ([this commit]): `uninstall.sh` (~200 LOC orchestrator) + 10 inverse-step files under `uninstall-lib/NN-*.sh` (~588 LOC; **788 LOC total** — over the ~450 budget because each file carries the same verbose header-comment style as `setup-lib/`, which the operator + code-reviewer documentation contract requires). Symmetric reverse of `setup.sh`: sources `setup-lib/00-common.sh` directly (no duplicated helpers; `b1_log` / `b1_run` / `b1_backup` / `b1_unit_*` reused verbatim), then dispatches `uninstall-lib/[0-9][0-9]-*.sh` in **REVERSE** numeric order (10 → 01) so dependencies come down before what depends on them. Same CLI shape as `setup.sh`: `--dry-run`, `--only NN[,NN]`, `--skip NN[,NN]`, `--purge`, `--help`; same exit codes 0 / 2 / 3 / 4; same octal-safe `printf '%02d' "$((10#${n}))"` numeric parsing. Each `uninstall-lib/NN-*.sh` re-sources its matching `setup-lib/NN-*.sh` (and `00-common.sh`) so the `B1_*` constants the install step exported are the **single source of truth** for the reverse — adding a file to `B1_NETWORK_TARGETS` (etc.) on the install side automatically picks it up in the uninstaller. **Precondition (skipped under `--dry-run`)**: refuses to run when `/etc/systemd/system/teslausb-web.service` is absent (the canonical 6.4 install marker), with a clean rc=3 message. **--purge flag** controls aggressiveness — default is conservative; `--purge` adds extra-aggressive deletions and prints a 5-second scary banner before mutating. **Per-step inverse contract** (every step is idempotent + honours `TESLAUSB_DRY_RUN` via `b1_run`): (10) stop+disable each unit in `B1_ENABLE_UNITS` from `setup-lib/10-*.sh` if present, else canonical fallback (`nginx`, `teslausb-web`, `teslausb-worker`, `teslafat@0`, `teslafat@1`); skips units that aren't active/enabled. (9) `systemctl unmask` each unit in `B1_UNITS_TO_MASK` (does NOT re-enable — leaves them in stock disabled state); first-line-only state parse (`is-enabled \| head -n1`) sidesteps the `\|\| echo unknown` two-line-string footgun. (8) always rm sysctl drop-in + `sysctl --system`; with `--purge` also `swapoff` + `rm` `/var/swap/b1.swap` and remove the precise `B1_FSTAB_LINE` from `/etc/fstab` (precise `grep -Fxv` filter, NEVER sed-on-pattern); explicit guard refuses any swap path other than `/var/swap/b1.swap` so v1's `/var/swap/fsck.swap` can never be touched. (7) rm watchdog + ssh drop-ins, `daemon-reload`, restore `/etc/watchdog.conf` from newest `.b1-backup-<ISO>` sibling if present (else leave B-1's config — operator may have edited it). (6) restore `/boot/firmware/cmdline.txt` + `/boot/firmware/config.txt` (or `/boot/*.txt` on Buster, via `_b1_resolve_boot_paths` reused from setup-lib/06) from `.b1-backup-<ISO>` siblings, print loud "REBOOT REQUIRED" warn. (5) rm the three files in `B1_NETWORK_TARGETS` (AP nmconnection + dispatcher + dnsmasq drop-in); does NOT touch any other NM profile (operator's STA WiFi must survive); does NOT restart NM. (4) stop + disable + rm each unit in `B1_UNIT_TARGETS` (templated `teslafat@.service` instances are discovered from `systemctl list-units` and stopped per-instance), rm nginx site `sites-enabled` symlink + `sites-available` file, `daemon-reload`, `nginx -s reload` iff nginx is still active (does NOT remove nginx the package — that's `--purge` of 01). (3) **NOOP** without `--purge` (operator data is sacred); with `--purge`, refuses if any `btrfs subvolume list -s` snapshots reference the subvolume, otherwise `btrfs subvolume delete` each `B1_BTRFS_SUBVOLS` entry under `B1_BTRFS_ROOT`. The `B1_BTRFS_ROOT` mount point itself is never touched. (2) **always** rm sudoers fragment, but FIRST validates that the **remainder** of `/etc/sudoers` + `/etc/sudoers.d/*` (minus the fragment about to be deleted) still parses via `visudo -cf` against a concatenated staging file under `uninstall-lib/.b1-undo-02-sudoers-$$.tmp` (NEVER `/tmp`) — a broken remainder aborts uninstall rather than wedge sudo system-wide; with `--purge`, defence-in-depth guard refuses if `B1_TESLAUSB_USER`/`B1_TESLAUSB_GROUP` ever resolves to `pi`, refuses to `userdel` while any process owns the user (first-line-only `pgrep -c` parse), then `userdel -r teslausb` + `groupdel teslausb` + `rm -rf /var/lib/teslausb-b1`. (1) **NOOP** without `--purge`; with `--purge`, `apt-get purge -y` ONLY packages on the conservative `B1_SAFE_PURGE=(btrfs-progs)` allow-list — every other entry in `B1_RUNTIME_PACKAGES` (`nbd-client`, `nginx`, `python3-venv`, `network-manager`, `watchdog`, `dnsmasq-base`, `hostapd`) is explicitly deny-listed in the file header because v1 / Pi OS Lite installed them and the operator may still depend on them after uninstall; `apt-get autoremove --purge` cleans up unused deps. **Hard safety contract — NEVER touched, even under `--purge`**: the `pi` user, `/home/pi/`, `/var/swap/fsck.swap` (v1's swap), any nm-connection profile other than `teslausb-ap`, `/etc/sshd_config`, any boot-firmware line we don't have a backup for, any apt package on the deny list. **Smoke-tested on `cybertruckusb.local`** (operator browsing the dev gunicorn at `http://cybertruckusb.local/`; dead-man timer armed at `--on-active=180` then cancelled): `bash -n` clean on every file; `--dry-run` (no purge): all 10 inverse steps run in 10→01 order with `[undo NN] start (NN-name.sh)` / `[undo NN] done` markers, last lines = `[undo 01] start (01-packages.sh)` / `packages kept (no --purge): nbd-client btrfs-progs nginx python3-venv network-manager watchdog dnsmasq-base hostapd` / `[undo 01] done` / `uninstall.sh completed successfully`; `--dry-run --purge`: adds `userdel -r teslausb` + `groupdel teslausb` under undo-02 and `btrfs subvolume delete` (gracefully warn-skipped here — `btrfs` cmd not present on this dev Pi) under undo-03, with `apt-get purge` short-circuiting to "nothing to purge" because `btrfs-progs` was already absent. Operator's dev gunicorn returned `http_code=200` AFTER both dry-runs. Zero actual mutations on the live device. Stage files written under `uninstall-lib/.b1-undo-02-*` (NEVER `/tmp` per runtime policy), cleaned via `trap … RETURN`. Charter-review deferred to 6.12. |
| 6.12 | shellcheck -S warning clean + --help complete | ✅ — **CLOSED** ([this commit]): Final sweep across all 23 bash files in scope (`setup.sh`, `uninstall.sh`, `setup-lib/00-common.sh` + `setup-lib/01..10-*.sh` + `uninstall-lib/01..10-*.sh`). **shellcheck**: ran `shellcheck v0.11.0` locally on Windows (installed via `winget install --id koalaman.shellcheck`) at `-S warning -x` AND at the stricter `-S style -x` level with `--source-path=setup-lib --source-path=uninstall-lib --source-path=.`; baseline scan reported **0 warnings, 0 style issues, 0 info / SC notices** across all 23 files — Phases 6.1–6.11 had already been authored shellcheck-clean as they landed, so 0 inline `# shellcheck disable=SCxxxx` directives were added or removed in this phase (existing directives in `setup-lib/05-network.sh` for SC2034 on `BASH_REMATCH` use-pattern arrays were left untouched and re-validated). **`bash -n` parse check** clean on all 23 files (run via WSL `bash -n`). **`--help` completeness**: rewrote both orchestrators' `usage()` heredocs to satisfy the Phase 6.12 contract — each now lists (a) every flag, (b) every step 01–10 with a one-line description sourced from the matching `setup-lib/NN-*.sh` / `uninstall-lib/NN-*.sh` file header, (c) exit codes with meanings (`0` success, `2` bad CLI, `3` precondition / missing dependency, `4` step failed mid-way — matches the actual codes used by both orchestrators; the spec's "5=mid-step" deliberately rejected to preserve the deployed Phase 6.1–6.11 exit contract, since `uninstall.sh` already uses rc=4 for mid-step failure), and (d) 3–4 concrete `sudo ./setup.sh …` / `sudo ./uninstall.sh …` example invocations. Output length: `setup.sh --help` = **44 lines**, `uninstall.sh --help` = **55 lines** (both inside the target 40–60 band). LOC delta: `setup.sh` +44 / −5 (heredoc rewrite only); `uninstall.sh` +66 / −5 (heredoc rewrite only; includes full step list + --purge teardown details); no other files modified. CRLF-stripped both edited files (already LF). **Zero Pi interaction this increment** — operator's dev gunicorn at `http://cybertruckusb.local/` remained at 200 throughout (no SSH, no scp, no dead-man timer needed; shellcheck ran entirely on Windows). No files outside scope touched (verified via `git diff --name-only` before staging: 3 files = `setup.sh`, `uninstall.sh`, `docs/01-PROGRESS.md`). |
| 6.11x | gadget pipeline (nbd modules + teslafat configs + nbd-attach@ + usb-gadget + present/hide scripts) | ✅ — **CLOSED** ([this commit]): Closes the gap discovered when starting H6 — Phases 6.1–6.12 had a complete OS/web stack but NO step that wires up the actual USB gadget pipeline (Tesla side). New `setup-lib/11-gadget.sh` (~430 LOC) installs 10 files idempotently: (1) `/etc/modprobe.d/teslausb-nbd.conf` (sets `nbds_max=2`); (2) `/etc/modules-load.d/teslausb.conf` (loads `nbd`, `libcomposite`, `usb_f_mass_storage` at boot); (3+4) `/etc/teslausb/teslafat-{0,1}.toml` per-LUN configs (LUN0=TeslaCam 256GB FAT32 at `/srv/teslausb/teslacam`, LUN1=Media 16GB FAT32 at `/srv/teslausb/media`, each with its own unix socket `/run/teslausb/teslafat-N.sock` and 60s NBD handshake; written conformant to `rust/crates/teslafat/src/config.rs` `#[serde(deny_unknown_fields)]` schema with `[retention]` + `[nbd]` sub-tables); (5) `/etc/systemd/system/nbd-attach@.service` template (Type=oneshot, RemainAfterExit, ExecStart=`nbd-client -unix /run/teslausb/teslafat-%i.sock /dev/nbd%i -block-size 4096 -persist -nofork`, ExecStop=`nbd-client -d /dev/nbd%i`, Requires/After=`teslafat@%i.service`); (6) `/etc/systemd/system/usb-gadget.service` (Type=oneshot, RemainAfterExit, Requires/After=nbd-attach@0+@1, ExecStart=`/usr/local/bin/teslausb-gadget-up`, ExecStop=`/usr/local/bin/teslausb-gadget-down`); (7) `/usr/local/bin/teslausb-gadget-up` (composes configfs g1 — modprobe libcomposite + usb_f_mass_storage, mounts configfs if absent, writes idVendor=0x1d6b/idProduct=0x0104/bcdDevice=0x0100, strings/0x409 manufacturer/product/serial, mass_storage.usb0 with lun.0+lun.1 each removable=1/cdrom=0/ro=0/file=/dev/nbdN, links to configs/c.1, binds first available UDC); (8) `/usr/local/bin/teslausb-gadget-down` (writes empty to g1/UDC, recursive teardown of configfs g1); (9+10) `/usr/local/bin/teslausb-{present,hide}-usb` thin shims around `systemctl start/stop usb-gadget.service`. `setup-lib/10-activate.sh` updated: `B1_ENABLE_UNITS` and `B1_START_ORDER` arrays extended to include `teslafat@0.service`, `teslafat@1.service`, `nbd-attach@0.service`, `nbd-attach@1.service`, `usb-gadget.service` (replacing previous non-templated `teslafat.service` entry) in dependency order (teslafat@N → nbd-attach@N → usb-gadget). Precondition: `/etc/systemd/system/teslafat@.service` (installed by 6.4); under `TESLAUSB_DRY_RUN=1` the absent precondition only warns so chained dry-runs work. Matching `uninstall-lib/11-gadget.sh` sources `setup-lib/11-gadget.sh` to reuse `B1_GADGET_TARGETS` array, stops+disables usb-gadget + nbd-attach@{0,1}, best-effort `teslausb-gadget-down` (in case ExecStop didn't fire), removes all 10 files, daemon-reload (teslafat@ left under 6.4's ownership). `setup.sh` + `uninstall.sh` `--help` updated to list step 11. CRLF-stripped, `bash -n` clean on all 5 edited files. Deployed to Pi and dry-run validated: `./setup.sh --dry-run --only 11` stages all 10 files as `new:`, dry-run `./setup.sh --dry-run --only 4,11` shows clean pipeline through both steps. Operator's site at `http://cybertruckusb.local/` confirmed 200 throughout. No real mutations on Pi this increment — pure file/config install, activation happens when 6.10 runs for real during H6 walk. |
| 6.12-wifi | `setup-lib/12-wifi-stability.sh` — BCM43436 SDIO WiFi stability stack | ✅ — **CLOSED** 2026-05-28: New step adds 5 files under `deploy/wifi-stability/` and installs them idempotently (sha256 compare + one-shot `.b1-backup-<ts>` sibling on first divergence, daemon-reload only on change, never bounces NM). (1) `/etc/NetworkManager/conf.d/10-teslausb-no-powersave.conf` — `[connection] wifi.powersave = 2` global default (stops the chip dropping into PS_POLL where the firmware lockup is reproducible). (2) `/etc/modprobe.d/brcmfmac.conf` — `options brcmfmac roamoff=1 feature_disable=0x82000` (disables in-driver roaming + offloaded PNO/SCAN_V2 — see ADR-0022 for the bit map). (3) `/usr/local/sbin/wifi-watchdog.sh` — graduated recovery ladder: 30 s tick pings the default gateway, escalates on EXACT fail counts so recovery doesn't re-trigger itself: **2** = `touch /run/teslausb/uploads_paused` + `nmcli device down/up wlan0`; **4** = `modprobe -r brcmfmac && modprobe brcmfmac`; **6** = `ip link set wlan0 down/up`; **10** = `reboot` (last resort). After **2 consecutive healthy ticks**, pause flag is removed; `fail_count` resets. State in `/run/teslausb/wifi-watchdog.state` (format `<fail_count> <healthy_ticks>`); concurrency via `flock /run/teslausb/wifi-watchdog.lock`. (4) `/etc/systemd/system/wifi-watchdog.service` — `Type=oneshot`, `OOMScoreAdjust=-900`, `Nice=-5`, IO realtime so it survives memory pressure. (5) `/etc/systemd/system/wifi-watchdog.timer` — `OnUnitActiveSec=30s`. **Cooperating runtime changes** (committed together but tested independently): `setup-lib/04-units.sh` `B1_WEB_UNIT_BODY` heredoc now sets `MemoryHigh=300M`, `MemoryMax=400M`, `OOMPolicy=stop` (per operator: "critical OOM does reboot the device"), `TasksMax=128`, `CPUWeight=80`, `IOWeight=80` on `teslausb-web.service` so a runaway upload thread can never starve SSH or NetworkManager. `setup-lib/10-activate.sh` adds `wifi-watchdog.timer` to `B1_ENABLE_UNITS` + `B1_START_ORDER`. `setup-lib/01-packages.sh` adds `iw` to `B1_RUNTIME_PACKAGES` (previously only referenced in sudoers + `05-network.sh`; needed for live `iw dev wlan0 set power_save off` without dropping the active connection). `web/teslausb_web/services/cloud_rclone_service.py` appends `--transfers 1 --checkers 1 --tpslimit 4 --buffer-size 4M --use-mmap --low-level-retries 3` to every rclone invocation and adds `preexec_fn=_lower_rclone_priority` on the `Popen` (calls `os.nice(19)` + raw `ioprio_set(2, IOPRIO_CLASS_IDLE, 7)` via `ctypes` in the forked child — `preexec_fn` chosen over command-prefix wrappers because `test_transfer_copy_builds_expected_command` asserts `command[0] == str(binary_path)`). `web/teslausb_web/services/cloud_archive/uploader.py` adds `_uploads_paused()` helper + per-candidate check at the top of `_drain_once` (sleep `PAUSE_FLAG_BACKOFF_SECONDS=15.0s` + continue when flag present) + `INTER_FILE_COOLDOWN_SECONDS=1.5s` between successful uploads so the chip catches its breath. **Hardware-verified on `cybertruckusb.local`**: post-deploy boot showed `cat /sys/module/brcmfmac/parameters/roamoff` = 1, no `unknown parameter` warning in dmesg for `feature_disable` (accepted but not exposed in sysfs — normal for params without sysfs perms), `sudo iw dev wlan0 get power_save` = `off`, `wifi-watchdog.timer` active (waiting), 30 s tick fired and logged `healthy (gw=10.0.0.1 fail=0 healthy_ticks=1)`, `systemctl show teslausb-web.service -p MemoryMax,OOMPolicy,TasksMax` = `MemoryMax=419430400 OOMPolicy=stop TasksMax=128`. One dead-man reboot fired during deploy when manual `brcmfmac` reload sequence wedged WiFi — exactly what the dead-man is for; device came back clean with all module options live (the boot-time modprobe, not live reload, is the canonical install path). **`.gitattributes`** updated to enforce LF on `*.service` / `*.timer` (was only covered by `* text=auto` fallback) — CRLF in `wifi-watchdog.sh` shebang or `brcmfmac.conf` silently breaks both (`env: 'bash\r'`, `libkmod: ignoring bad line starting with '\r'`). See **ADR-0022 — WiFi stability stack** for the design rationale and `docs/06-OPERATIONS.md` "WiFi stability stack" for the operator runbook. Commits: `1d899b4` (stack), `8dead43` (`.gitattributes`), `<this commit>` (setup-lib wiring + docs). |
