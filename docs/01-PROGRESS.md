...
| 6.3 | setup.sh btrfs subvolume creation | ✅ — **CLOSED** ([this commit]): `setup-lib/03-btrfs.sh` (175 LOC) creates the two B-1 data subvolumes `/srv/teslausb/teslacam` and `/srv/teslausb/media` idempotently. Constants `B1_BTRFS_ROOT` + `B1_BTRFS_SUBVOLS=(teslacam media)` + owner/group/mode exported at file scope for 6.11 (uninstall.sh) to reuse verbatim. Detection via `stat -f -c %T` (btrfs vs other); existing subvols detected via `btrfs subvolume show`; existing-as-plain-dir → hard refuse (no auto-delete of operator data). If `/srv/teslausb` parent is not btrfs → clean `b1_err` telling operator to mount a btrfs volume first (NO mkfs, NO mount, NO rootfs conversion). If `btrfs` cmd absent (called before 6.1 via `--only 03`) → `b1_warn` + graceful skip. If `teslausb` group absent (called before 6.2) → chown deferred with warn; subvols still created. Ownership chown only fires when `stat -c %U:%G:%a` doesn't already match `teslausb:teslausb:775` (group-writable for `pi` once 6.2 adds it to group). Every mutation through `b1_run`; probes run even under dry-run for accurate reporting. **Smoke-tested on `cybertruckusb.local`**: (1) without btrfs-progs → warn-and-skip path clean; (2) with btrfs stub + real ext4 `/srv` → "exists but is on 'ext2/ext3', not btrfs" hard fail with rc=1 (the intentional dev-Pi path). `bash -n` clean. Charter-review deferred to 6.12. |
