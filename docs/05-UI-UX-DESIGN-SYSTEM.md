...
## B-1 status (2026-05-20)

This document was copied verbatim from the v1 `main` branch at commit
`75bfca0` (`docs/UI_UX_DESIGN_SYSTEM.md`) and lightly edited for the
B-1 architecture. **The visual design, interaction patterns, layout,
colors, typography, spacing, animation curves, accessibility rules,
empty-state copy, and information architecture all carry over
unchanged** — per the operator's binding constraint: *"I want the
website to look the same as it does now."*

The edits in this B-1 copy are limited to the small list of changes
...
2. **The `--mode-present` / `--mode-edit` semantic color tokens are
   renamed to `--samba-off` / `--samba-on`.** Same hex values, same
   roles — the dot in the nav now means "Samba enabled" rather than
   "Edit mode active". When Samba is disabled in config, the dot is
   hidden entirely.
3. **`--accent-warning` is no longer documented as "edit mode".** It
   still means "warning" generally.
4. **Feature pages are no longer gated by IMG file existence.** B-1
   has no IMG files. Every media page (Lock Chimes, Light Shows,
   Music, Boombox, Wraps, License Plates) is always reachable; pages
   with an empty backing folder show an upload-prompt empty state.
5. **Settings → Mode Control is removed.** Replaced by Settings →
   Network Sharing (Samba on/off toggle). The "Mode switch on boot"
   toggle, "Quick edit timeout" slider, "Boot fsck enabled" toggle,
...
7. **Lock chime / light show / music / wraps upload flows no longer
   ...
   schedule cache invalidation`. The user sees no difference.

Everything else in this document is binding for B-1 exactly as it
was for v1. The screenshot-diff acceptance gate (Phase H5) requires
zero visible delta between v1 and B-1 on any shared surface.
...
|-------|--------------------|--------------------|
| **Glanceable** | Map with recent trip, network-sharing dot (when Samba enabled), trip summary | None — visible on load |
...
| `--accent-success` | `#22C55E` | Success indicators |
...
|-------|------|-------|-------|
| `--samba-off` | `#22C55E` | `#16A34A` | Samba disabled — Tesla has exclusive access to the USB (the happy default) |
| `--samba-on` | `#F59E0B` | `#D97706` | Samba enabled — both Tesla and the local network can write to the USB |
...
In B-1 there is no Present/Edit mode distinction — Tesla always sees the USB. The nav dot now reflects Samba state only, and is hidden entirely when Samba is disabled in config.

**Normal state (Samba disabled — default):**
- No dot in the nav. The USB is Tesla-only.

**Network sharing active (Samba enabled in Settings → Network Sharing):**
- Amber dot + `"Sharing"` label in the nav (`--samba-on` token)
- Persistent subtle banner: `"Network Sharing active — files written over SMB will appear on Tesla after a brief sync"` (no "USB disconnected" copy — Tesla still sees the USB in B-1; SMB writes are propagated via cache invalidation)
- Banner has a "Stop sharing" button that flips the toggle off

**Error/unknown state (worker unreachable, IPC timeout):**
- Red dot + `"Worker offline"` label
...
Media (/media/)      — Lock Chimes, Music, Boombox, Light Shows, Wraps, License Plates (sub-tabs, always present; each shows an upload-prompt empty state when its backing folder is empty)
Settings (/settings/) — Network sharing toggle, WiFi, AP, system config
```
...
- Network-sharing dot at bottom (amber = Samba on; hidden when Samba off — see Status Indicator above)
