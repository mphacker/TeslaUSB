...
| P0 | Cloud status/control | `cloud_archive.py` sync/status routes | `/api/cloud` via uploadd `get_status`; `CloudArchive.tsx` status banner; mutations pending | webd + uploadd | uploader status snapshot | local-socket read-only status; mutations require approved auth/CSRF and durable jobs | Partial | status route/socket tests and Cloud Archive UAT; later Sync Now, wake, stop, cancel |
...
| P0 | Cleanup control | cleanup status/policy/preview/execute/report | Read-only `/api/retention/status` (governor, bounded candidate totals/bytes, bounded recent cleanup history, cloud-durability disclosure), capped `/api/retention/preview`, and Storage diagnostics retention report wiring; policy/execute remain planned | webd + retentiond + indexd + SPA | retention policy and job history | shared server-side eligibility predicate; caps are explicit; cloud durability disclosure is explicit; mutations require protected jobs | Partial | retention status/preview Rust tests + Storage Health UAT; execute, cancel, report pending |
...
| P1 | Wi-Fi onboarding | captive portal status/connect/saved profiles | `/captive-portal` now consumes read-only `/api/wifi/status`, `/networks`, `/saved`; mutations pending | wifid + webd + SPA | NetworkManager profiles | serialized mutation, rollback, link-loss recovery | Partial | read-only desktop/mobile UAT; live onboarding mutation UAT pending |
...
| P1 | Failed-job retry/delete | jobs counts/retry/delete + `specs/contracts/durable-mutation-jobs.md` | `/api/jobs`, `/api/jobs/failed`, `/api/jobs/failed/uploads`, read-only `/api/jobs/capabilities`; shared mutation envelope/validation helpers | webd + indexd + owning daemon | durable attempt history + indexd v9 retry-request identity scaffold (`cloud_failed_upload_retry_requests`) | requestId/idempotencyKey/requestHash/jobId envelope validation; idempotency same key+hash replay + different-hash `409`; strict Host/Origin/Sec-Fetch checks required for new durable mutation routes (legacy mutation routes may still use older checks); bounded redacted errors; recording/handoff exclusion; failed-upload retry contract is child-specific and `failed`-only (reject `done|queued|in_progress|parked`) with optional `upload_set_id` generation fence semantics (sealed rows require match, unsealed rows require omission); retry/delete routes still disabled | Partial | durable-mutation state/validation tests + capabilities route test + failed-upload history route tests + indexd retry-request persistence tests; retry/delete IPC and UAT pending |
