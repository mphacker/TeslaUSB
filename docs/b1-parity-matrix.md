...
| P1 | Failed-job retry/delete | jobs counts/retry/delete + `specs/contracts/durable-mutation-jobs.md` | `/api/jobs`, `/api/jobs/failed`, read-only `/api/jobs/capabilities`; shared mutation envelope/validation helpers | webd + owning daemon | durable attempt history | requestId/idempotencyKey/requestHash/jobId envelope validation; idempotency same key+hash replay + different-hash `409`; strict Host/Origin/Sec-Fetch checks required for new durable mutation routes (legacy mutation routes may still use older checks); bounded redacted errors; recording/handoff exclusion; retry/delete still disabled | Partial | durable-mutation state/validation tests + capabilities route test; retry/delete IPC and UAT pending |
