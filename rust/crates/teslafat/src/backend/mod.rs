...
//!   Walks the operator-supplied `backing_root`, plans an `exFAT`
//!   layout against the configured volume size, and serves
//!   byte-level reads by composing the in-memory metadata
//!   synthesizers with on-demand backing-file reads.
...
//!   wires it into `SynthBackend::write` via [`exfat_write`].
//! * [`exfat_write`] — `ExfatWriteState`: Phase 3.5e write state
//!   machine for `exFAT`. Uses crash-safe `.partial` rename
//!   discipline; carries `PartialEntrySet` between dir-cluster
//!   boundaries and short-circuits the FAT walk for the common
//!   `NoFatChain == true` extents.
//! * `pending_spill` — `PendingSpill`: bounded-FIFO spill
//!   buffer for data-cluster writes that arrive before their
//!   owning file or FAT chain is known. Used by `exfat_write` to
//!   prevent the unbounded growth that OOM-killed `teslafat` on
//!   the Pi Zero 2 W on 2026-05-24.
