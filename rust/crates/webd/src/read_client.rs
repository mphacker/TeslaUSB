//! `webd` client-side transport for `scannerd` `ReadFile`.
//!
...
///
/// Only consumed by the Unix client wiring in `lib.rs`; `cfg`-gated so the
/// non-Unix host build (used by the SPA UAT harness) doesn't flag it dead.
#[cfg(unix)]
pub const SCANNERD_READ_SOCKET_PATH: &str = "/run/teslausb/scannerd-read.sock";
/// Maximum JSON control frame size (header/request).
///
/// Used only by the Unix transport and the wire tests; `cfg`-gated to keep the
/// non-Unix host build clean under `-D unused`.
#[cfg(any(unix, test))]
...
/// `ReadFile` failures.
///
/// On a non-Unix, non-test build only [`ReadFileError::Io`] is constructed:
...
/// Fallback client used on non-Unix hosts (and as an explicit unavailable stub
/// in tests) where the Unix socket transport is not available.
#[derive(Debug, Clone, Copy, Default)]
pub struct UnavailableReadFileClient;

impl ReadFileClient for UnavailableReadFileClient {
    fn read_file(&self, _: &ReadFileRequest) -> Result<ReadFileOk, ReadFileError> {
        Err(ReadFileError::Io(io::Error::new(
            io::ErrorKind::Unsupported,
            "scannerd read socket is unavailable on this platform",
        )))
    }
}
