...
def _find_front_camera_videos(media_root: Path) -> Iterator[Path]:
...
def _iter_front_camera_videos_with_mtime(media_root: Path) -> Iterator[tuple[Path, float]]:
    for path in _find_front_camera_videos(media_root):
        try:
